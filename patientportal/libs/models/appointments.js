...
function create(clinicianID, patientID, dateTime) {
    ...
    if(clinicianID && patientID && dateTime) {
        return new Promise(async function (resolve, reject) {
            try {
                ...
                if (valid) {
                    let result = await collection.insertOne({clinicianID: clinicianID, patientID: patientID, dateTime: dateTime, duration: duration});
    ...
    if (appointmentID && newDateTime) {
        return new Promise( async function (resolve, reject) {
            try {
                ...
                let valid = true;
                let appointment = await collection.find({_id: appointmentID}).toArray();
                if (!appointment) { 
                    ...
                    if (valid) {
                        let success = await collection.updateOne({_id: appointmentID}, {$set: {dateTime: newDateTime}});
...
function getPatientSchedule(userID) {
    ...
    return new Promise( async function (resolve, reject) {
        try {
            ...
            // Get the documents collection
            let collection = db.collection(collectionName);
            let allAppointments = await collection.find({patientID: userID}).toArray();
            allAppointments.sort((a, b) => {return new Date(a.dateTime) < new Date(b.dateTime)});
            resolve(allAppointments);
        } catch (e) {
            reject(e);
...
function getClinicianSchedule(userID) {
    ...
    return new Promise( async function (resolve, reject) {
        try {
            ...
            // Get the documents collection
            let collection = db.collection(collectionName);
            let allAppointments = await collection.find({clinicianID: userID}).toArray();
            allAppointments.sort((a, b) => {return new Date(a.dateTime) < new Date(b.dateTime)});
            resolve(allAppointments);
        } catch (e) {
            reject(e);
...
module.exports = {create, update, cancel, getSlotsTaken, getPatientSchedule, getClinicianSchedule};